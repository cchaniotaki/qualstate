...
async function addState(session, page, STATES_SPA_COMPARE, IDS_IGNORE_SPA_COMPARE, STATES_SPA_EVALUATION,
                        selector, xpathM, loggerM) {
    ...
    if (STATES_SPA_COMPARE.has(bodyCompareHash)) {
        ...
        fs.writeFile(filePath, bodyEvaluation.toString(), 'utf8', (err) => {
            if (err) {
